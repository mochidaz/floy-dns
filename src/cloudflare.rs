...
impl Cloudflare {
    ...
    pub async fn add_subdomain_dns_record(&self, subdomain: &String, ip: &String) -> Result<(), ErrorKind> {
        let name = format!("{}.{}", subdomain, &self.config.dns_suffix);

        let client = &self.client;
        let url = format!("https://api.cloudflare.com/client/v4/zones/{}/dns_records", &self.config.cf_zone_id);

        let body= DnsRecord::new(
            "A".to_owned(),
            name,
            1,
            ip.to_owned(),
    ...
    pub async fn get_subdomain_dns_record(&self, subdomain: &String, wildcard: bool) -> Result<String, ErrorKind> {
        ...
        let url = format!("https://api.cloudflare.com/client/v4/zones/{}/dns_records?type=A&name={}", &self.config.cf_zone_id, name);
    ...
    pub async fn update_subdomain_dns_record(&self, subdomain: &String, ip: &String) -> Result<(), ErrorKind> {
        ...
        let wildcard_res = client.put(&url)
            .bearer_auth(&self.config.cf_api_key)
...
impl<'r> FromRequest<'r> for CloudflareGuard {
    ...
    async fn from_request(request: &'r Request<'_>) -> Outcome<CloudflareGuard, ErrorKind> {
        let cloudflare = request.guard::<&rocket::State<Config>>()
            .await
            .unwrap();

        Outcome::Success(CloudflareGuard(Cloudflare::new(Config::from(cloudflare)).await))
    }
}
